...
def generate_sql_plan(question: str, intent: str, resolved_schema: 'ResolvedSchema', history: list[dict]) -> QueryPlan:
    ...
    prompt = PromptTemplate.from_template(
        ...
        "═══ RULES ═══\n"
        "1. ONLY use tables and columns that appear in the schema above. NEVER invent tables or columns.\n"
        "2. If there are multiple tables, use JOINs when the question requires data from different tables.\n"
        ...
        "7. CRITICAL: SQLite is CASE-SENSITIVE for string values. Always use LOWER() for text comparisons.\n"
        "   e.g. WHERE LOWER(category) = 'groceries' instead of WHERE category = 'Groceries'.\n"
        "   Apply this to ALL WHERE, HAVING, and LIKE clauses that filter on text columns.\n"
        "8. WINDOW FUNCTIONS: When using SUM() OVER, AVG() OVER, ROW_NUMBER() etc.,\n"
        "   ALWAYS apply WHERE filters FIRST in a CTE or subquery, THEN compute the window function on the filtered result.\n"
        "   Never compute a window function on the unfiltered table and then filter.\n"
        "9. DATE FILTERS: When the user says 'last N days/months', compute the cutoff relative to MAX(date) in the table,\n"
        "   not relative to today's date.\n"
        "10. SPENDING/EXPENSES: When calculating spending, expenses, or income, do NOT assume amounts are negative (e.g. amount < 0).\n"
        "    In many datasets, all amounts are positive. Instead, filter using the transaction type column if it exists (e.g. LOWER(type) IN ('debit', 'expense')).\n\n"
